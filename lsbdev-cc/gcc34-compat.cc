...
/*
 * The following struct is based on libstdc++'s unwind-cxx.h. 
 * It is used to calculate the proper size for the C++ exception object.
 * Most type information has been stripped out of the struct for pointers,
 * since pointers are the same size no matter the destination type. 
 */
 ...
 * as possible. 
 */
void *fake_cxa_get_exception_ptr(void *exc_obj_in) throw()
{
...
extern "C" void *__cxxabiv1::
__cxa_get_exception_ptr(void *exc_obj_in) throw()
{
    void *cpplib;
    ...
    /* Load the library's ABI function. If it's not there, use the fake one. */
    dlerror();
